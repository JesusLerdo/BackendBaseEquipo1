...
const modelsEquipo = require("../models/equipo")
const {queryEquipoExists} = require("../models/equipo")

const addequip = async (req = request, res = response) => {
    const {Nombre,
          Ciudad,
          Pais, 
          Dueño, 
    ...
    try {
        ...
        if(equipExist){
            res.status(400).json({msg: `El Equipo '${Nombre}' ya se encuentra registrado`})
            return
        }
                 //generamos la consulta
                    const result = await conn.query(modelsEquipo.queryAddEquip,
                            [
                            Nombre,
                            Ciudad,
                            Pais, 
                    ...
                    res.status(404).json({msg: `No se pudo reagistrar el equipo con Nombre ${Nombre}`})
                    return
                    }
                    res.json({msg:`Se agregó satisfactoriamente el Equipo con Nombre ${Nombre}`})//Se manda la lista de equipos
    }catch (error){
        console.log(error)
...
module.exports = {addequip, updateEquipo, getEquipo, getEquipoByID, deleteEquipoByID}